...
namespace BigInt
{
    public class BigInt : BigNumber
    {
        ...
        public bool IsProbablePrime(int Rounds = 50)
        {
            BigInt P = this.Clone();
        ...
        public bool IsPrimeFermat(BigInt Base, bool SkipVerification = false)
        {
            ...
            if (APowModP != 1)
                //if (A < P && SkipVerification)
                //{
                //    // 3.1. Calculate A^P mod P
                //    APowModP = A.PowModPrim(P, P);

                //    // 3.2. If APowModP != A then P is not prime
                //    if (APowModP != A)
                //        return false;
                //    else
                //        return true;
        ...
        public bool IsPrimeMR(BigInt[] Bases)
        {
            ...
            for (int i = 0; i < Bases.Length; i++)
            {
                ...
                if (EgcAP.Spart == 1 && EgcAP.Value[0] != 1)
                    return false;
                if (!this.IsPrimeMR(Bases[i], true))
                    return false;
            }
            return true;
        }
